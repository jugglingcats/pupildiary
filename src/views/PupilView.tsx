...
function formatDate(
    date: { date?: string | undefined; dateTime?: string | undefined; timeZone?: string | undefined },
    includeTime: boolean = false
) {
    return moment(date.date || date.dateTime).format(includeTime ? "ddd D MMM YYYY, h:mma" : "D MMM YYYY")
}
...
type CalendarItemProps = {
    calendar: Required<gapi.client.calendar.CalendarListEntry>
            ...
            {props.events.map(e =>
                isLesson(e) ? (
                    <Grid.Row key={e.id} className={dateBeforeNowFilter(e) ? "" : "future-lesson"}>
                        <Grid.Column computer={3} mobile={7}>
                            {formatDate(e.start!)} {overrideReason(e)}
                        </Grid.Column>
                        <Grid.Column textAlign="right" computer={2} mobile={4}>
                            <LessonEntry
                                event={e}
                                defaultAmount={props.amount}
                                onOverride={(amount, reason) => props.onOverrideLesson(e, amount, reason)}
                    ...
                    <Grid.Row key={e.id} className="payment-row">
                        ...
                        <Grid.Column textAlign="right" computer={2} mobile={5}>
                            <PaymentEntry
                                event={e}
                                onUpdate={amount => props.onUpdatePayment(e, amount)}
                                onDelete={() => props.onDeletePayment(e)}
                            />
                        </Grid.Column>
                    </Grid.Row>
                )
...
export const PupilView = (props: CalendarItemProps) => {
    ...
    const updateEvent = (lesson: Required<gapi.client.calendar.Event>, extendedProperties, suffix?) => {
        setLoading(true)
        return attempt(
            gapi.client.calendar.events
                .update({
                    ...
                    resource: {
                        summary:
                            suffix !== undefined
                                ? `${nickname ? nickname : name}${suffix.length ? " - " + suffix : ""}`
                                : lesson.summary,
    ...
    const overrideLesson = (
        lesson: Required<gapi.client.calendar.Event>,
    ...
    const resetLesson = (lesson: Required<gapi.client.calendar.Event>) => {
        return updateEvent(lesson, {}, "")
    }

    const updatePayment = (e: Required<gapi.client.calendar.Event>, amount: number) => {
        return updateEvent(e, { private: { paymentAmount: amount } })
    }

    const deletePayment = (e: Required<gapi.client.calendar.Event>) => {
        setLoading(true)
        return attempt(
            gapi.client.calendar.events
                .delete({
    ...
    useEffect(() => {
        ...
        async function fetch_all_events(): Promise<gapi.client.calendar.Event[]> {
            let pageToken: string | undefined = undefined
            const events: gapi.client.calendar.Event[] = []
    ...
    return (
        <div className="student">
            <div
                key="header"
                onClick={headerClick}
                className={"student-header-row" + (mode !== CalendarMode.none ? " active" : "")}
            >
                <span className="student-name">{nickname || name}</span>
                <span style={{ whiteSpace: "nowrap" }}>
                    {(loading && <Loader active={true} inline={true} size="tiny" />) || (
                        <span>
                            <MenuLink mode={CalendarMode.lessons}>Balance</MenuLink>
                            <Label basic color={balance >= 0 ? "green" : "red"}>
                                {balance}
                            </Label>
                            <MenuSeparator />
                    ...
                    {
                        {
                            [CalendarMode.lessons]: (
                                <Fragment>
                                    <Grid columns={4} className="compact">
                                            ...
                                            <Grid.Column width={16}>
                                                <Form>
                                                    <Form.Group className="create-payment">
                                                        ...
                                                        <Form.Button disabled={!validPayment()} onClick={addPayment}>
                                                            Add
                                                        </Form.Button>
                                                        <Form.Button onClick={() => setShowAddPayment(false)}>
                                                            Cancel
                                                        </Form.Button>
                                                    </Form.Group>
                                                </Form>
                                        ...
                                        {events.length === 0 ? (
                                            <div>
                                                No lessons or payments found. Please create at least one calendar entry
                                                for this student in Google Calendar to get started (for example a
                                                recurring appointment), after which past lessons will appear here.
                                            </div>
                                        ) : (
                                            <BalanceGrid
                                                events={events.sort(compareDates)}
                                                amount={amount}
                                                onOverrideLesson={overrideLesson}
                            ...
                            [CalendarMode.settings]: (
                                <Grid columns={1} className="compact">
                                    <Grid.Column width={16}>
                                        <CreateStudent
                                        ...
                                        <Segment>
                                            ...
                                            <p>
                                                Before deleting the calendar for this student you can create a backup by
                                                downloading the data as a CSV file.
                                            </p>
                                            <Card.Group>
                                                <Card>
                                                    <Card.Content>Download student data</Card.Content>
                                                    <Card.Content extra>
                                                        <Button
                                                            ...
                                                            size="medium"
                                                            as="a"
                                                            href={events_dl(props.calendar, events)}
                                                            download={`${initials}-tutoring.csv`}
                                                        >
                                                            Download
                                                            <Icon name="download" />
                                                        </Button>
                                                        <span className="download-desc">
                                                            {events.length} lessons/payments
                                                        </span>
                                                    </Card.Content>
                                                </Card>
                                            </Card.Group>
